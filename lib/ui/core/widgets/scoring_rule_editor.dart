...
class _ScoringRuleEditorState extends State<ScoringRuleEditor> {
  ...
  void initState() {
    ...
    _conditionController =
        TextEditingController(text: widget.rule.condition);
    _formulaController =
        TextEditingController(text: widget.rule.formula ?? '');
    _hasSubRules = widget.rule.rules != null;

    _conditionFocus.addListener(() {
      if (_conditionFocus.hasFocus) widget.onFocus?.call(_conditionController);
    });
    _formulaFocus.addListener(() {
      if (_formulaFocus.hasFocus) widget.onFocus?.call(_formulaController);
    });
  }
  ...
  void didUpdateWidget(ScoringRuleEditor oldWidget) {
    ...
    if (!_conditionFocus.hasFocus &&
        widget.rule.condition != _conditionController.text) {
      _conditionController.value = _conditionController.value.copyWith(
        text: widget.rule.condition,
        selection:
            TextSelection.collapsed(offset: widget.rule.condition.length),
      );
    }
    if (!_formulaFocus.hasFocus &&
        widget.rule.formula != null &&
        widget.rule.formula != _formulaController.text) {
      _formulaController.value = _formulaController.value.copyWith(
        text: widget.rule.formula!,
        selection:
            TextSelection.collapsed(offset: widget.rule.formula!.length),
      );
    }
              ...
              children: [
                ...
                DropdownButton<bool>(
                  ...
                  items: const [
                    ...
                    DropdownMenuItem(
                        value: true, child: Text('Sous-conditions')),
                  ],
                  onChanged: (val) {
                    if (val != null && val != _hasSubRules) {
                      setState(() => _hasSubRules = val);
              ...
              Padding(
                ...
                child: Column(
                  children: [
                    if (widget.rule.rules != null)
                      ...widget.rule.rules!.asMap().entries.map((entry) {
                        ...
                        return ScoringRuleEditor(
                          ...
                          onChanged: (updated) {
                            final newRules =
                                List<ScoringRule>.from(widget.rule.rules!);
                    ...
                    OutlinedButton.icon(
                      onPressed: () {
                        final newRules = List<ScoringRule>.from(
                            widget.rule.rules ?? []);
